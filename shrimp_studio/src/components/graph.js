...
const DFgraph = () => {
	...
	const fetchData = async () => {
		...
		const listFilesAndDirectories = async (parentRef) => {
			nodes.push({ id: parentRef.name, name: "", group: 1, radius: 2 });
			const { items, prefixes } = await listAll(parentRef);



			items.map((item) => {
				nodes.push({ id: parentRef.name + item.name, name: item.name, group: 2, radius: 2 });
				links.push({ source: parentRef.name, target: parentRef.name + item.name, value: 1 });
			});
	...
	useEffect(() => {
		...
		const javascriptRetarded = async () => {
